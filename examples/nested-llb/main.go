...
func main() {
	frontend := llb.Frontend(
		llb.Image("docker.io/openllb/llb-mkfile:latest", imagemetaresolver.WithDefault),
		llb.WithInput("foo", llb.Scratch().File(
			llb.Mkfile("/out", 0600, []byte("foo")),
		)),
		llb.WithInput("bar", llb.Scratch().File(
			llb.Mkfile("/out", 0600, []byte("bar")),
		)),
	)

	dt, err := frontend.Marshal(llb.LinuxAmd64)
	if err != nil {
		panic(fmt.Sprintf("failed marshalling frontend: %s", err))
	}
